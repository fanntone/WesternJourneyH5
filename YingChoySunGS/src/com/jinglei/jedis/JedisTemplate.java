...
/**
 * JedisTemplate 提供了一個template方法，負責對Jedis連接的獲取與歸還。
 * JedisAction<T> 和 JedisActionNoResult兩種回調接口，適用干有無友回值兩種情況。
 * PipelineAction 與 PipelineActionResult兩種接口，適用於pipeline中批量傳輸命令的情況。
 * 
 * 同時提供一些JedisOperation中定義的最常用函數的封裝 最常用函數的封裝，如Get/Set/zadd等。
 */
